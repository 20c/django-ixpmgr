...
class _ProxyFieldMixin:
    ...
    """
    # todo: cleaner mixin constructor?
    def _init(self, manager, field_name, relation, proxy_model):
        self._manager = manager
        self.source_field_name = field_name
        self.source_relation = relation
    ...
    def contribute_to_class(self, model, name, **_):
        ...
        if contrib: contrib(model, name)
        setattr(model, name, self)
        mgr = self.get_manager(model)
        mgr.add_proxy_field(name, self.source_field_name)
...
class _ConstFieldDescriptor:
    ...
    def contribute_to_class(self, model, name, **_):
        setattr(model, name, self)
        self.manager.add_proxy_field(name, None)
...
class ProxyManager(models.Manager):
    ...
    def const_field(self, value):
        return _ConstFieldDescriptor(self, value)
    def null_field(self):
        return _ConstFieldDescriptor(self, None)

    def field(self, source, *args, relation=None, proxy_model=None, **kwargs):
        ...
        proxy._init(self, field_name, relation, proxy_model)
        return proxy
